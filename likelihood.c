...
/*----------------------------------------------------------------
|
|   CondLikeDown_Bin_AVX: binary model with or without rate
|       variation
|
-----------------------------------------------------------------*/
int CondLikeDown_Bin_AVX (TreeNode *p, int division, int chain)
{
    ...
    for (k=0; k<m->numGammaCats; k++)
        {
        for (c=0; c<m->numAVXChars; c++)
            {
            m1 = _mm256_set1_ps (tiPL[0]);
            m2 = _mm256_set1_ps (tiPR[0]);
            m5 = _mm256_mul_ps (m1, clL[0]);
            m6 = _mm256_mul_ps (m2, clR[0]);

            m1 = _mm256_set1_ps (tiPL[1]);
            m2 = _mm256_set1_ps (tiPR[1]);
            ...
            *clP++ = _mm256_mul_ps (m5, m6);
            ...
            m1 = _mm256_set1_ps (tiPL[2]);
            m2 = _mm256_set1_ps (tiPR[2]);
            m5 = _mm256_mul_ps (m1, clL[0]);
            m6 = _mm256_mul_ps (m2, clR[0]);

            m1 = _mm256_set1_ps (tiPL[3]);
            m2 = _mm256_set1_ps (tiPR[3]);
            m3 = _mm256_mul_ps (m1, clL[1]);
            m4 = _mm256_mul_ps (m2, clR[1]);
...
//        sem_wait(&clmix_nuc4_go_sem);

		if(endrunchain == 1)
        {
//            printf("ThreadCLMix_NUC4 No.%d exit.\n",id);
            pthread_exit(NULL);
        }
        switch(option)
        {
            case CLDOWN:
                ThreadCLDown_Gen(id);
//                sem_post(&cld_nuc4_stop_sem);
                break;
            case CLROOT:
                ThreadCLRoot_Gen(id);
//                sem_post(&clr_nuc4_stop_sem);
                break;
            case CLSCALER:
                ThreadCLScaler_Gen(id);
//                sem_post(&cls_nuc4_stop_sem);
...
#ifdef DEBUGG
		MrBayesPrint("Thread %d beyond barrier \n", id);
#endif
    }
}
#endif
...
#   endif
    ...
    {
        cld_gen_prmt[i].c = 0;
        cld_gen_prmt[i].shortCut = shortCut;
        cld_gen_prmt[i].pL = pL;
        cld_gen_prmt[i].pR = pR;
        cld_gen_prmt[i].clP = clP;
        cld_gen_prmt[i].clL = clL;
        cld_gen_prmt[i].clR = clR;
        cld_gen_prmt[i].lState = lState;
        cld_gen_prmt[i].rState = rState;
		cld_gen_prmt[i].nStates = nStates;
		cld_gen_prmt[i].preLikeJump = preLikeJump;
		cld_gen_prmt[i].numChars = m->numChars;
		cld_gen_prmt[i].numGammaCats = m->numGammaCats;
		
        for(j=0;j<i;j++)
        {
		cld_gen_prmt[i].c += charNum[j];
		cld_gen_prmt[i].clL += (charNum[j]*nStates);
		cld_gen_prmt[i].clR += (charNum[j]*nStates);
		cld_gen_prmt[i].clP += (charNum[j]*nStates);
		}
	}
    ...
    if(on_off == 0)
    {
        ...
        {
            clmix_gen_id[i] = i;
            pthread_create(&clmix_gen_thd[i],NULL,ThreadCLMix_Gen,(void *)&clmix_gen_id[i]);
        }		
    }
    pthread_barrier_wait(&start_barrier);
#endif
	...
	switch (shortCut)
        ...
        case 0:
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                tiPR += nStatesSquared;
#ifdef FINE_GRAINED
				clL += m->numChars*nStates - charNum[0]*nStates;
				clR += m->numChars*nStates - charNum[0]*nStates;
				clP += m->numChars*nStates - charNum[0]*nStates;
#endif
                }
            break;
        case 1:
...
{
    int             a, b, c, h, i, k, j, shortCut, *lState=NULL, *rState=NULL,
                    nStates, nStatesSquared, preLikeJump;
    CLFlt           likeL, likeR, *pL, *pR, *tiPL, *tiPR, *clL, *clR, *clP;
	pL = cld_gen_prmt[id].pL;
    pR = cld_gen_prmt[id].pR;
    clL = cld_gen_prmt[id].clL;
    clR = cld_gen_prmt[id].clR;
    clP = cld_gen_prmt[id].clP;
    nStates = cld_gen_prmt[id].nStates;
    lState = cld_gen_prmt[id].lState;
    rState = cld_gen_prmt[id].rState;
	shortCut = cld_gen_prmt[id].shortCut;
    preLikeJump = cld_gen_prmt[id].preLikeJump;
	nStatesSquared = nStates*nStates;

	int numChars = cld_gen_prmt[id].numChars;
	int numGammaCats = cld_gen_prmt[id].numGammaCats;
    ...
    switch (shortCut)
        ...
        case 0:
            ...
            for (k=0; k<numGammaCats; k++)
                {
                for (c=cld_gen_prmt[id].c; c<(cld_gen_prmt[id].c+charNum[id]); c++)
                    {
                    for (i=h=0; i<nStates; i++)
                        {
                        likeL = likeR = 0.0;
                        ...
                        *(clP++) = likeL * likeR;
#ifdef DEBUGG
						MrBayesPrint("Thread %d clP at %d = %d\n",id, clP-1,*(clP-1));
#endif
                        }
                    clL += nStates;
                    clR += nStates;
...
#ifdef FINE_GRAINED
				clL += numChars*nStates - charNum[id]*nStates;
				clR += numChars*nStates - charNum[id]*nStates;
				clP += numChars*nStates - charNum[id]*nStates;
#endif
                tiPL += nStatesSquared;
                tiPR += nStatesSquared;
                }
...
/*----------------------------------------------------------------
|
|   CondLikeDown_Gen_AVX: general n-state model with or without rate
|       variation
|
-----------------------------------------------------------------*/
int CondLikeDown_Gen_AVX (TreeNode *p, int division, int chain)
{
    ...
    CLFlt           *pL, *pR, *tiPL, *tiPR;
    __m256          *clL, *clR, *clP;
    __m256          mTiPL, mTiPR, mL, mR, mAcumL, mAcumR;
    ModelInfo       *m;
    CLFlt           *preLikeRV[FLOATS_PER_VEC];
    CLFlt           *preLikeLV[FLOATS_PER_VEC];
...
#   endif
	...
	{
		cld_gen_AVX_prmt[i].clL = clL;
		cld_gen_AVX_prmt[i].clR = clR;
		cld_gen_AVX_prmt[i].clP = clP;
		cld_gen_AVX_prmt[i].pL = pL;
		cld_gen_AVX_prmt[i].pR = pR;
		cld_gen_AVX_prmt[i].c = 0;
		cld_gen_AVX_prmt[i].numAVXChars = m->numAVXChars;
		cld_gen_AVX_prmt[i].numGammaCats = m->numGammaCats;
		cld_gen_AVX_prmt[i].shortCut = shortCut;
		cld_gen_AVX_prmt[i].lState = lState;
        cld_gen_AVX_prmt[i].rState = rState;
		cld_gen_AVX_prmt[i].nStates = nStates;
		cld_gen_AVX_prmt[i].preLikeJump = preLikeJump;
		for(int j=0;j<i;++j)
		{
			cld_gen_AVX_prmt[i].c+=charNum[j];
			cld_gen_AVX_prmt[i].clL+=(nStates*charNum[j]);
			cld_gen_AVX_prmt[i].clR+=(nStates*charNum[j]);
			cld_gen_AVX_prmt[i].clP+=(nStates*charNum[j]);
		}
	}
	if(on_off==0)
	{	
		...
		pthread_barrier_init(&start_barrier,NULL,NUM_THREADS);
		pthread_barrier_init(&stop_barrier,NULL,NUM_THREADS);
		pthread_barrier_init(&half_barrier,NULL,NUM_THREADS);
		datatype = GEN;
		for(int i=1; i<NUM_THREADS; i++)
		{
			clmix_gen_id[i] = i;
			pthread_create(&clmix_gen_AVX_thd[i],NULL,ThreadCLMix_Gen_AVX,(void *)&clmix_gen_id[i]);
		}
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=0; c<m->numAVXChars; c++)
#endif
                    {
                     for (i=h=0; i<nStates; i++)
                        {
                        mAcumL = _mm256_setzero_ps();
                        mAcumR = _mm256_setzero_ps();
                        for (j=0; j<nStates; j++)
                            {
                            mTiPL  = _mm256_set1_ps (tiPL[h]);
                            mTiPR  = _mm256_set1_ps (tiPR[h++]);
                            mL     = _mm256_mul_ps (mTiPL, clL[j]);
                            mR     = _mm256_mul_ps (mTiPR, clR[j]);
                            mAcumL = _mm256_add_ps (mL, mAcumL);
                            mAcumR = _mm256_add_ps (mR, mAcumR);
                            }
                        *(clP++) = _mm256_mul_ps (mAcumL, mAcumR);
                        }
                    clL += nStates;
                    clR += nStates;
                    }
                ...
                tiPR += nStatesSquared;
#ifdef FINE_GRAINED
				clL += m->numAVXChars*nStates - charNum[0]*nStates;
				clR += m->numAVXChars*nStates - charNum[0]*nStates;
				clP += m->numAVXChars*nStates - charNum[0]*nStates;
#endif
                }
            break;
        case 1:
            ...
            for (k=0; k<m->numGammaCats; k++)
                {
#ifdef FINE_GRAINED
				for (c=t=0; c<charNum[0]; c++)
#else
                for (c=t=0; c<m->numAVXChars; c++)
#endif
                    {
                    for (c1=0; c1<FLOATS_PER_VEC; c1++,t++)
                        {
                    ...
                    for (i=h=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following statment we assume that AVX register can hold exactly 8 ClFlts. */
                        mAcumL = _mm256_set_ps (*(preLikeLV[7]++), *(preLikeLV[6]++), *(preLikeLV[5]++), *(preLikeLV[4]++),
												*(preLikeLV[3]++), *(preLikeLV[2]++), *(preLikeLV[1]++), *(preLikeLV[0]++));
                        mAcumR = _mm256_setzero_ps();
                        for (j=0; j<nStates; j++)
                            {
                            mTiPR  = _mm256_set1_ps (tiPR[h++]);
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=h=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following statment we assume that AVX register can hold exactly 8 ClFlts. */
                        mAcumR = _mm256_set_ps (*(preLikeRV[7]++), *(preLikeRV[6]++), *(preLikeRV[5]++), *(preLikeRV[4]++),
												*(preLikeRV[3]++), *(preLikeRV[2]++), *(preLikeRV[1]++), *(preLikeRV[0]++));
                        mAcumL = _mm256_setzero_ps();
                        for (j=0; j<nStates; j++)
                            {
                            mTiPL  = _mm256_set1_ps (tiPL[h++]);
        ...
        case 3:
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following 2 statments we assume that AVX register can hold exactly 8 ClFlts. */
                        mL = _mm256_set_ps (*(preLikeLV[7]++), *(preLikeLV[6]++), *(preLikeLV[5]++), *(preLikeLV[4]++),
											*(preLikeLV[3]++), *(preLikeLV[2]++), *(preLikeLV[1]++), *(preLikeLV[0]++));
                        mR = _mm256_set_ps (*(preLikeRV[7]++), *(preLikeRV[6]++), *(preLikeRV[5]++), *(preLikeRV[4]++),
											*(preLikeRV[3]++), *(preLikeRV[2]++), *(preLikeRV[1]++), *(preLikeRV[0]++));
                        *(clP++) = _mm256_mul_ps (mL,mR);
                        }
                    }
#ifdef FINE_GRAINED
				clP += m->numAVXChars*nStates - charNum[0]*nStates;
...
#if defined (AVX_ENABLED) && (FINE_GRAINED)
int ThreadCLDown_Gen_AVX(int id)
{
	int             c, c1, h, i, j, k, t, shortCut, *lState=NULL, *rState=NULL, nStates, nStatesSquared, preLikeJump;
    CLFlt           *pL, *pR, *tiPL, *tiPR;
    __m256          *clL, *clR, *clP;
    __m256          mTiPL, mTiPR, mL, mR, mAcumL, mAcumR;
    CLFlt           *preLikeRV[FLOATS_PER_VEC];
	...
	clL = cld_gen_AVX_prmt[id].clL;
	clR = cld_gen_AVX_prmt[id].clR;
	clP = cld_gen_AVX_prmt[id].clP;
	pL = cld_gen_AVX_prmt[id].pL;
	pR = cld_gen_AVX_prmt[id].pR;
	int numAVXChars = cld_gen_AVX_prmt[id].numAVXChars;
	int numGammaCats = cld_gen_AVX_prmt[id].numGammaCats;
	shortCut = 	cld_gen_AVX_prmt[id].shortCut;
	lState = cld_gen_AVX_prmt[id].lState;
    rState = cld_gen_AVX_prmt[id].rState;
	nStates = cld_gen_AVX_prmt[id].nStates;
	preLikeJump = cld_gen_AVX_prmt[id].preLikeJump;
	nStatesSquared = nStates * nStates;
    ...
    switch (shortCut)
        ...
        case 0:
            ...
            for (k=0; k<numGammaCats; k++)
                {
                for (c=cld_gen_AVX_prmt[id].c; c<(cld_gen_AVX_prmt[id].c+charNum[id]); c++)
                    {
                    for (i=h=0; i<nStates; i++)
                        {
                        mAcumL = _mm256_setzero_ps();
				...
				clL += numAVXChars*nStates - charNum[id]*nStates;
				clR += numAVXChars*nStates - charNum[id]*nStates;
				clP += numAVXChars*nStates - charNum[id]*nStates;
        ...
        case 1:
            ...
            for (k=0; k<numGammaCats; k++)
                {
				for (c=cld_gen_AVX_prmt[id].c, t=0; c<(cld_gen_AVX_prmt[id].c+charNum[id]); c++)
                    {
                    for (c1=0; c1<FLOATS_PER_VEC; c1++,t++)
                        {
                        preLikeLV[c1] = &preLikeL[lState[t] + k*(preLikeJump+nStates)];
...
#if defined (FINE_GRAINED) && (AVX_ENABLED)
void       *ThreadCLMix_Gen_AVX(void *prmt)
{
    ...
    while(1)
    {
        ...
        {
            case CLDOWN:
                ThreadCLDown_Gen_AVX(id);
                break;
            case CLROOT:
                ThreadCLRoot_Gen_AVX(id);
                break;
            case CLSCALER:
                ThreadCLScaler_Gen_AVX(id);
                break;
        }
		pthread_barrier_wait(&stop_barrier);
    }
...
//            printf("ThreadCLMix_NUC4 No.%d exit.\n",id);
        ...
        {
            case CLDOWN:
                ThreadCLDown_NUC4(id);
//                sem_post(&cld_nuc4_stop_sem);
                break;
            case CLROOT:
                ThreadCLRoot_NUC4(id);
//                sem_post(&clr_nuc4_stop_sem);
                break;
            case CLSCALER:
                ThreadCLScaler_NUC4(id);
//                sem_post(&cls_nuc4_stop_sem);
                break;
			case LIKEH:
				ThreadLikeH_NUC4(id);
...
#   endif
    ...
    for(i=1; i<NUM_THREADS; i++)
    {
        ...
        cld_nuc4_prmt[i].k = 0;
		cld_nuc4_prmt[i].h = 0;
        cld_nuc4_prmt[i].shortCut = shortCut;
        cld_nuc4_prmt[i].pL = pL;
        cld_nuc4_prmt[i].pR = pR;
        cld_nuc4_prmt[i].clP = clP;
        cld_nuc4_prmt[i].clL = clL;
        cld_nuc4_prmt[i].clR = clR;
        cld_nuc4_prmt[i].lState = lState;
        cld_nuc4_prmt[i].rState = rState;
		cld_nuc4_prmt[i].numChars = m->numChars;
		cld_nuc4_prmt[i].numGammaCats = m->numGammaCats;
        for(j=0;j<i;j++)
        {
		cld_nuc4_prmt[i].c += charNum[j];
		cld_nuc4_prmt[i].h += (4*charNum[j]);
		cld_nuc4_prmt[i].clL += (4*charNum[j]);
		cld_nuc4_prmt[i].clR += (4*charNum[j]);
	}
/*	if(on_off == 0)
	{
		printf("cld_nuc4_prmt[%d].c = %d\n",i,cld_nuc4_prmt[i].c);
	}
*/
    }
/*    for(i=1; i<NUM_THREADS; i++)
    {
        sem_post(&clmix_nuc4_go_sem);
    }*/
    if(on_off == 0)
    {
    on_off = 1;
	...
	datatype = NUC4;
        ...
        {
            clmix_nuc4_id[i] = i;
            pthread_create(&clmix_nuc4_thd[i],NULL,ThreadCLMix_NUC4,(void *)&clmix_nuc4_id[i]);
        }
    }
    pthread_barrier_wait(&start_barrier);
#endif
    ...
    switch (shortCut)
        ...
        case 0:
            ...
            for (k=h=0; k<m->numGammaCats; k++)
                ...
                tiPR += 16;
#ifdef FINE_GRAINED
				clL += m->numChars*4 - charNum[0]*4;
				clR += m->numChars*4 - charNum[0]*4;
				h += m->numChars*4 - charNum[0]*4;
#endif
                }
            break;
        case 1:
...
void       ThreadCLDown_NUC4 (int id)
{
    int             c, h, i, j, k, shortCut, *lState=NULL, *rState=NULL;
    CLFlt           *clL, *clR, *clP, *pL, *pR, *tiPL, *tiPR;
	pL = cld_nuc4_prmt[id].pL;
    pR = cld_nuc4_prmt[id].pR;
    clL = cld_nuc4_prmt[id].clL;
    clR = cld_nuc4_prmt[id].clR;
    clP = cld_nuc4_prmt[id].clP;
    lState = cld_nuc4_prmt[id].lState;
    rState = cld_nuc4_prmt[id].rState;
	shortCut = cld_nuc4_prmt[id].shortCut;
	h = cld_nuc4_prmt[id].h;
    //k = cld_gen_prmt[id].k;
	//c = cld_gen_prmt[id].c;
	int numChars = cld_nuc4_prmt[id].numChars;
	int numGammaCats = cld_nuc4_prmt[id].numGammaCats;
    switch (shortCut)
        {
        case 0:
            tiPL = pL;
            ...
            for (k=0; k<numGammaCats; k++)
                {
                for (c=cld_nuc4_prmt[id].c; c<(cld_nuc4_prmt[id].c+charNum[id]); c++)
                    {                    
                    clP[h++] =   (tiPL[AA]*clL[A] + tiPL[AC]*clL[C] + tiPL[AG]*clL[G] + tiPL[AT]*clL[T])
                                *(tiPR[AA]*clR[A] + tiPR[AC]*clR[C] + tiPR[AG]*clR[G] + tiPR[AT]*clR[T]);
                    clP[h++] =   (tiPL[CA]*clL[A] + tiPL[CC]*clL[C] + tiPL[CG]*clL[G] + tiPL[CT]*clL[T])
				...
				clL += numChars*4 - charNum[id]*4;
				clR += numChars*4 - charNum[id]*4;
				h += numChars*4 - charNum[id]*4;
...
/*----------------------------------------------------------------
|
|   CondLikeDown_NUC4_SSE: 4by4 nucleotide（核苷酸） model with or without rate
...
void       *ThreadCLMix_NUC4_AVX(void *prmt)
{
    ...
    while(1)
    {
        ...
        {
            case CLDOWN:
                ThreadCLDown_NUC4_AVX(id);
                break;
            case CLROOT:
                ThreadCLRoot_NUC4_AVX(id);
                break;
            case CLSCALER:
                ThreadCLScaler_NUC4_AVX(id);
                break;		
        }
		pthread_barrier_wait(&stop_barrier);
    }
...
int CondLikeDown_NUC4_AVX (TreeNode *p, int division, int chain)
{
	...
	for(int i=1;i<NUM_THREADS;++i)
	{
		cld_nuc4_AVX_prmt[i].clL = clL;
		cld_nuc4_AVX_prmt[i].clR = clR;
		cld_nuc4_AVX_prmt[i].clP = clP;
		cld_nuc4_AVX_prmt[i].pL = pL;
		cld_nuc4_AVX_prmt[i].pR = pR;
		cld_nuc4_AVX_prmt[i].c = 0;
		cld_nuc4_AVX_prmt[i].numAVXChars = m->numAVXChars;
		cld_nuc4_AVX_prmt[i].numGammaCats = m->numGammaCats;
		for(int j=0;j<i;++j)
		{
			cld_nuc4_AVX_prmt[i].c+=charNum[j];
			cld_nuc4_AVX_prmt[i].clL+=(4*charNum[j]);
			cld_nuc4_AVX_prmt[i].clR+=(4*charNum[j]);
			cld_nuc4_AVX_prmt[i].clP+=(4*charNum[j]);
		}
	}
	if(on_off==0)
	{	
		...
		{
			clmix_nuc4_id[i] = i;
			pthread_create(&clmix_nuc4_AVX_thd[i],NULL,ThreadCLMix_NUC4_AVX,(void *)&clmix_nuc4_id[i]);
		}
    ...
    for (k=0; k<m->numGammaCats; k++)
        ...
        for (c=0; c<m->numAVXChars; c++)
#endif 
            {
            m1 = _mm256_set1_ps (tiPL[AA]);
            m2 = _mm256_set1_ps (tiPR[AA]);
            m5 = _mm256_mul_ps (m1, clL[A]);
            m6 = _mm256_mul_ps (m2, clR[A]);

            m1 = _mm256_set1_ps (tiPL[AC]);
            m2 = _mm256_set1_ps (tiPR[AC]);
            m3 = _mm256_mul_ps (m1, clL[C]);
            m4 = _mm256_mul_ps (m2, clR[C]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[AG]);
            m2 = _mm256_set1_ps (tiPR[AG]);
            m3 = _mm256_mul_ps (m1, clL[G]);
            m4 = _mm256_mul_ps (m2, clR[G]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[AT]);
            m2 = _mm256_set1_ps (tiPR[AT]);
            ...
            *clP++ = _mm256_mul_ps (m5, m6);
            ...
            m1 = _mm256_set1_ps (tiPL[CA]);
            m2 = _mm256_set1_ps (tiPR[CA]);
            m5 = _mm256_mul_ps (m1, clL[A]);
            m6 = _mm256_mul_ps (m2, clR[A]);

            m1 = _mm256_set1_ps (tiPL[CC]);
            m2 = _mm256_set1_ps (tiPR[CC]);
            m3 = _mm256_mul_ps (m1, clL[C]);
            m4 = _mm256_mul_ps (m2, clR[C]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[CG]);
            m2 = _mm256_set1_ps (tiPR[CG]);
            m3 = _mm256_mul_ps (m1, clL[G]);
            m4 = _mm256_mul_ps (m2, clR[G]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[CT]);
            m2 = _mm256_set1_ps (tiPR[CT]);
            m3 = _mm256_mul_ps (m1, clL[T]);
            m4 = _mm256_mul_ps (m2, clR[T]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);
            ...
            *clP++ = _mm256_mul_ps (m5, m6);
            ...
            m1 = _mm256_set1_ps (tiPL[GA]);
            m2 = _mm256_set1_ps (tiPR[GA]);
            m5 = _mm256_mul_ps (m1, clL[A]);
            m6 = _mm256_mul_ps (m2, clR[A]);

            m1 = _mm256_set1_ps (tiPL[GC]);
            m2 = _mm256_set1_ps (tiPR[GC]);
            m3 = _mm256_mul_ps (m1, clL[C]);
            m4 = _mm256_mul_ps (m2, clR[C]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[GG]);
            m2 = _mm256_set1_ps (tiPR[GG]);
            m3 = _mm256_mul_ps (m1, clL[G]);
            m4 = _mm256_mul_ps (m2, clR[G]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[GT]);
            m2 = _mm256_set1_ps (tiPR[GT]);
            m3 = _mm256_mul_ps (m1, clL[T]);
            m4 = _mm256_mul_ps (m2, clR[T]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);
            ...
            *clP++ = _mm256_mul_ps (m5, m6);
            ...
            m1 = _mm256_set1_ps (tiPL[TA]);
            m2 = _mm256_set1_ps (tiPR[TA]);
            m5 = _mm256_mul_ps (m1, clL[A]);
            m6 = _mm256_mul_ps (m2, clR[A]);

            m1 = _mm256_set1_ps (tiPL[TC]);
            m2 = _mm256_set1_ps (tiPR[TC]);
            m3 = _mm256_mul_ps (m1, clL[C]);
            m4 = _mm256_mul_ps (m2, clR[C]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[TG]);
            m2 = _mm256_set1_ps (tiPR[TG]);
            m3 = _mm256_mul_ps (m1, clL[G]);
            m4 = _mm256_mul_ps (m2, clR[G]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);

            m1 = _mm256_set1_ps (tiPL[TT]);
            m2 = _mm256_set1_ps (tiPR[TT]);
            m3 = _mm256_mul_ps (m1, clL[T]);
            m4 = _mm256_mul_ps (m2, clR[T]);
            m5 = _mm256_add_ps (m3, m5);
            m6 = _mm256_add_ps (m4, m6);
        ...
        tiPR += 16;
#ifdef FINE_GRAINED
		clL += 4*m->numAVXChars - 4*charNum[0];
		clR += 4*m->numAVXChars - 4*charNum[0];
		clP += 4*m->numAVXChars - 4*charNum[0];
#endif        
		}
#ifdef FINE_GRAINED
	pthread_barrier_wait(&stop_barrier);
...
/*----------------------------------------------------------------
|
|   ThreadCLDown_NUC4_AVX: 4by4 nucleotide model with or without rate
...
int ThreadCLDown_NUC4_AVX (int id)
{
	...
	pL = cld_nuc4_AVX_prmt[id].pL;
	pR = cld_nuc4_AVX_prmt[id].pR;
	clL = cld_nuc4_AVX_prmt[id].clL;
	clR = cld_nuc4_AVX_prmt[id].clR;
	clP = cld_nuc4_AVX_prmt[id].clP;
	int numAVXChars = cld_nuc4_AVX_prmt[id].numAVXChars;
	int numGammaCats = cld_nuc4_AVX_prmt[id].numGammaCats;
	
    tiPL = pL;
    tiPR = pR;
    for (k=0; k<numGammaCats; k++)
        {
		for(c = cld_nuc4_AVX_prmt[id].c; c<(cld_nuc4_AVX_prmt[id].c+charNum[id]);c++)
            {
            m1 = _mm256_set1_ps (tiPL[AA]);
            m2 = _mm256_set1_ps (tiPR[AA]);
            m5 = _mm256_mul_ps (m1, clL[A]);
        ...
        tiPL += 16;
        tiPR += 16;
		clL += 4*numAVXChars - 4*charNum[id];
		clR += 4*numAVXChars - 4*charNum[id];
		clP += 4*numAVXChars - 4*charNum[id];
        }
...
/*----------------------------------------------------------------
|
|   CondLikeDown_NY98_AVX: codon model with omega variation
|
-----------------------------------------------------------------*/
        ...
        case 3:
            for (k=0; k<m->numOmegaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following 2 statments we assume that SSE register can hold exactly 4 ClFlts. */
                        mL = _mm256_set_ps (*(preLikeLV[7]++), *(preLikeLV[6]++), *(preLikeLV[5]++), *(preLikeLV[4]++),
                                            *(preLikeLV[3]++), *(preLikeLV[2]++), *(preLikeLV[1]++), *(preLikeLV[0]++));
                        mR = _mm256_set_ps (*(preLikeRV[7]++), *(preLikeRV[6]++), *(preLikeRV[5]++), *(preLikeRV[4]++),
                                            *(preLikeRV[3]++), *(preLikeRV[2]++), *(preLikeRV[1]++), *(preLikeRV[0]++));
...
/*----------------------------------------------------------------
|
|   CondLikeRoot_Bin_AVX:binary model with or without rate
|       variation 
|
-----------------------------------------------------------------*/
int CondLikeRoot_Bin_AVX (TreeNode *p, int division, int chain)
{
    ...
    for (k=0; k<m->numGammaCats; k++)
        ...
        for (c=0; c<m->numAVXChars; c++)
            ...
            m1 = _mm256_set1_ps (tiPL[0]);
            m5 = *clL++;
            m2 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPL[2]);
            m6 = _mm256_mul_ps (m1, m5);

            m1 = _mm256_set1_ps (tiPL[1]);
            m5 = *clL++;
            m3 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPL[3]);
            m5 = _mm256_mul_ps (m1, m5);

            m4 = _mm256_add_ps (m2, m3); /* in m4 we get (tiPL[0]*clL[0] + tiPL[1]*clL[1]) */
            m6 = _mm256_add_ps (m5, m6); /* in m6 we get (tiPL[2]*clL[0] + tiPL[3]*clL[1]) */

            m1 = _mm256_set1_ps (tiPR[0]);
            m5 = *clR++;
            m2 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPR[2]);
            m7 = _mm256_mul_ps (m1, m5);

            m1 = _mm256_set1_ps (tiPR[1]);
            m5 = *clR++;
            m3 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPR[3]);
            m5 = _mm256_mul_ps (m1, m5);

            m1 = _mm256_add_ps (m2, m3); /* in m1 we get (tiPR[0]*clR[0] + tiPR[1]*clR[1]) */
            m7 = _mm256_add_ps (m5, m7); /* in m7 we get (tiPR[2]*clR[0] + tiPR[3]*clR[1]) */

            m4 = _mm256_mul_ps (m1, m4); /* in m4 we get (tiPL[0]*clL[0] + tiPL[1]*clL[1])*(tiPR[0]*clR[0] + tiPR[1]*clR[1]) */
            m7 = _mm256_mul_ps (m6, m7); /* in m7 we get (tiPL[2]*clL[0] + tiPL[3]*clL[1])*(tiPR[2]*clR[0] + tiPR[3]*clR[1]) */

            m1 = _mm256_set1_ps (tiPA[0]);
            m5 = *clA++;
            m2 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPA[2]);
            m6 = _mm256_mul_ps (m1, m5);

            m1 = _mm256_set1_ps (tiPA[1]);
            m5 = *clA++;
            m3 = _mm256_mul_ps (m1, m5);
            m1 = _mm256_set1_ps (tiPA[3]);
            m1 = _mm256_mul_ps (m1, m5);

            m2 = _mm256_add_ps (m2, m3); /* in m1 we get (tiPA[0]*clA[0] + tiPA[1]*clA[1]) */
            m1 = _mm256_add_ps (m1, m6); /* in m1 we get (tiPA[2]*clA[0] + tiPA[3]*clA[1]) */

            *clP++ = _mm256_mul_ps (m2, m4);
            *clP++ = _mm256_mul_ps (m1, m7);
...
#   endif
	...
	{
		clr_gen_prmt[i].c = 0;
		clr_gen_prmt[i].pL = pL;
		clr_gen_prmt[i].pR = pR;
		clr_gen_prmt[i].pA = pA;
		clr_gen_prmt[i].clA = clA;
		clr_gen_prmt[i].clR = clR;
		clr_gen_prmt[i].clP = clP;
		clr_gen_prmt[i].clL = clL;
		clr_gen_prmt[i].nStates = nStates;
		clr_gen_prmt[i].lState = lState;
		clr_gen_prmt[i].rState = rState;
		clr_gen_prmt[i].aState = aState;
		clr_gen_prmt[i].shortCut = shortCut;
		clr_gen_prmt[i].preLikeJump = preLikeJump;
		clr_gen_prmt[i].k = k;
		clr_gen_prmt[i].numChars = m->numChars;
		clr_gen_prmt[i].numGammaCats = m->numGammaCats;
		for(j = 0; j < i; ++j)
		{			
			clr_gen_prmt[i].c += charNum[j];
			clr_gen_prmt[i].clL += charNum[j]*nStates;
			clr_gen_prmt[i].clA += charNum[j]*nStates;
			clr_gen_prmt[i].clR += charNum[j]*nStates;
			clr_gen_prmt[i].clP += charNum[j]*nStates;
		}	
	}
	if(on_off==0)
		{
    ...
    switch (shortCut)
        ...
        case 4:
            ...
            for (k=0; k<m->numGammaCats; k++)
				...
				clR += m->numChars*nStates - charNum[0]*nStates;
				clP += m->numChars*nStates - charNum[0]*nStates;
				clA += m->numChars*nStates - charNum[0]*nStates;
#endif
                tiPL += nStatesSquared;
                tiPR += nStatesSquared;
                tiPA += nStatesSquared;
...
void       ThreadCLRoot_Gen (int id)
{
	...
	shortCut = clr_gen_prmt[id].shortCut;
	lState = clr_gen_prmt[id].lState;
	rState = clr_gen_prmt[id].rState;
	aState = clr_gen_prmt[id].aState;
	nStates = clr_gen_prmt[id].nStates;
	preLikeJump = clr_gen_prmt[id].preLikeJump;
	nStatesSquared = nStates*nStates;
	clL = clr_gen_prmt[id].clL;
	clR = clr_gen_prmt[id].clR;
	clA = clr_gen_prmt[id].clA;
	clP = clr_gen_prmt[id].clP;
	pL = clr_gen_prmt[id].pL;
	pR = clr_gen_prmt[id].pR;
	pA = clr_gen_prmt[id].pA;
	//k = clr_gen_prmt[id].k;
	//c = clr_gen_prmt[id].c;
	int numChars = clr_gen_prmt[id].numChars;
	int numGammaCats = clr_gen_prmt[id].numGammaCats;
	//for debug
    //shortCut = 0;
    switch (shortCut)
        {
        case 4:
            ...
            for (k=0; k<numGammaCats; k++)
                {
                for (c=clr_gen_prmt[id].c; c<(clr_gen_prmt[id].c+charNum[id]); c++)
                    {
                    for (i=h=0; i<nStates; i++)
                        {
                        likeL = likeR = likeA = 0.0;
				...
				clR += numChars*nStates - charNum[id]*nStates;
				clP += numChars*nStates - charNum[id]*nStates;
				clA += numChars*nStates - charNum[id]*nStates;
#endif
                tiPL += nStatesSquared;
                tiPR += nStatesSquared;
                tiPA += nStatesSquared;
...
#ifdef DEBUGG
	MrBayesPrint("Thread %d finished \n",id);
#endif
    return;
}
...
/*----------------------------------------------------------------
|
|   CondLikeRoot_Gen_AVX:general n-state model with or without rate
|       variation
|
-----------------------------------------------------------------*/
int CondLikeRoot_Gen_AVX (TreeNode *p, int division, int chain)
{
    ...
    CLFlt           *pL, *pR, *pA,
                    *tiPL, *tiPR, *tiPA;
    __m256          *clL, *clR, *clP, *clA;
    __m256          mTiPL, mTiPR, mTiPA, mL, mR, mA, mAcumL, mAcumR, mAcumA;
    ModelInfo       *m;
    CLFlt           *preLikeRV[FLOATS_PER_VEC];
    CLFlt           *preLikeLV[FLOATS_PER_VEC];
    CLFlt           *preLikeAV[FLOATS_PER_VEC];
...
#   endif
	...
	for(int i=1;i<NUM_THREADS;++i)
	{
		clr_gen_AVX_prmt[i].clL = clL;
		clr_gen_AVX_prmt[i].clR = clR;
		clr_gen_AVX_prmt[i].clP = clP;
		clr_gen_AVX_prmt[i].clA = clA;
		clr_gen_AVX_prmt[i].pL = pL;
		clr_gen_AVX_prmt[i].pR = pR;
		clr_gen_AVX_prmt[i].pA = pA;
		clr_gen_AVX_prmt[i].c = 0;
		clr_gen_AVX_prmt[i].numAVXChars = m->numAVXChars;
		clr_gen_AVX_prmt[i].numGammaCats = m->numGammaCats;
		clr_gen_AVX_prmt[i].shortCut = shortCut;
		clr_gen_AVX_prmt[i].lState = lState;
        clr_gen_AVX_prmt[i].rState = rState;
		clr_gen_AVX_prmt[i].aState = aState;
		clr_gen_AVX_prmt[i].nStates = nStates;
		clr_gen_AVX_prmt[i].preLikeJump = preLikeJump;
		for(int j=0;j<i;++j)
		{
			clr_gen_AVX_prmt[i].c+=charNum[j];
			clr_gen_AVX_prmt[i].clL+=(nStates*charNum[j]);
			clr_gen_AVX_prmt[i].clR+=(nStates*charNum[j]);
			clr_gen_AVX_prmt[i].clP+=(nStates*charNum[j]);
			clr_gen_AVX_prmt[i].clA+=(nStates*charNum[j]);
		}
	}
	if(on_off==0)
	{	
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=0; c<m->numAVXChars; c++)
                    ...
                    for (i=h=0; i<nStates; i++)
                        ...
                        mAcumL = _mm256_setzero_ps();
                        mAcumR = _mm256_setzero_ps();
                        mAcumA = _mm256_setzero_ps();
                        for (j=0; j<nStates; j++)
                            {
                            mTiPL  = _mm256_set1_ps (tiPL[h]);
                            mTiPR  = _mm256_set1_ps (tiPR[h]);
                            mTiPA  = _mm256_set1_ps (tiPA[h++]);
                            mL     = _mm256_mul_ps (mTiPL, clL[j]);
                            mR     = _mm256_mul_ps (mTiPR, clR[j]);
                            mA     = _mm256_mul_ps (mTiPA, clA[j]);
                            mAcumL = _mm256_add_ps (mL, mAcumL);
                            mAcumR = _mm256_add_ps (mR, mAcumR);
                            mAcumA = _mm256_add_ps (mA, mAcumA);
                            }
                        mAcumL = _mm256_mul_ps (mAcumL, mAcumR);
                        *(clP++) = _mm256_mul_ps (mAcumL, mAcumA);
                        }
                    clL += nStates;
                    clR += nStates;
                    clA += nStates;
...
#ifdef FINE_GRAINED
				clL += m->numAVXChars*nStates - charNum[0]*nStates;
				clA += m->numAVXChars*nStates - charNum[0]*nStates;
				clR += m->numAVXChars*nStates - charNum[0]*nStates;
				clP += m->numAVXChars*nStates - charNum[0]*nStates;
#endif
                }
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=h=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following statment we assume that AVX register can hold exactly 8 ClFlts. */
                        mAcumA = _mm256_set_ps (*(preLikeAV[7]++), *(preLikeAV[6]++), *(preLikeAV[5]++), *(preLikeAV[4]++),
											 *(preLikeAV[3]++), *(preLikeAV[2]++), *(preLikeAV[1]++), *(preLikeAV[0]++));
                        mAcumL = _mm256_setzero_ps();
                        mAcumR = _mm256_setzero_ps();
                        for (j=0; j<nStates; j++)
                            {
            ...
            for (k=0; k<m->numGammaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=0; i<nStates; i++)
                        ...
                        mR = _mm256_set_ps (*(preLikeRV[7]++), *(preLikeRV[6]++), *(preLikeRV[5]++), *(preLikeRV[4]++),
											*(preLikeRV[3]++), *(preLikeRV[2]++), *(preLikeRV[1]++), *(preLikeRV[0]++));
                        mA = _mm256_set_ps (*(preLikeAV[7]++), *(preLikeAV[6]++), *(preLikeAV[5]++), *(preLikeAV[4]++),
											*(preLikeAV[3]++), *(preLikeAV[2]++), *(preLikeAV[1]++), *(preLikeAV[0]++));
                        mL = _mm256_mul_ps (mL,mR);
                        *(clP++) = _mm256_mul_ps (mL,mA);
                        }
                    }
#ifdef FINE_GRAINED
				clP += m->numAVXChars*nStates - charNum[0]*nStates;
...
/*----------------------------------------------------------------
|
|   ThreadCLRoot_Gen_AVX:general n-state model with or without rate
|       variation
|
-----------------------------------------------------------------*/
int ThreadCLRoot_Gen_AVX(int id)
{
	int             c, c1, t, h, i, j, k, shortCut, *lState=NULL, *rState=NULL, *aState=NULL, preLikeJump,
                    nStates, nStatesSquared;
    CLFlt           *pL, *pR, *pA,
                    *tiPL, *tiPR, *tiPA;
    __m256          *clL, *clR, *clP, *clA;
	...
	clL = clr_gen_AVX_prmt[id].clL;
	clR = clr_gen_AVX_prmt[id].clR;
	clP = clr_gen_AVX_prmt[id].clP;
	clA = clr_gen_AVX_prmt[id].clA;
	pL = clr_gen_AVX_prmt[id].pL;
	pR = clr_gen_AVX_prmt[id].pR;
	pA = clr_gen_AVX_prmt[id].pA;
	int numAVXChars = clr_gen_AVX_prmt[id].numAVXChars;
	int numGammaCats = clr_gen_AVX_prmt[id].numGammaCats;
	shortCut = clr_gen_AVX_prmt[id].shortCut;
	lState = clr_gen_AVX_prmt[id].lState;
    rState = clr_gen_AVX_prmt[id].rState;
	aState = clr_gen_AVX_prmt[id].aState;
	nStates = clr_gen_AVX_prmt[id].nStates;
	preLikeJump = clr_gen_AVX_prmt[id].preLikeJump;
	nStatesSquared = nStates * nStates;
	...
	case 4:
		...
		for (k=0; k<numGammaCats; k++)
			{
			for (c=clr_gen_AVX_prmt[id].c; c<(clr_gen_AVX_prmt[id].c+charNum[id]); c++)
				{
				for (i=h=0; i<nStates; i++)
					{
					mAcumL = _mm256_setzero_ps();
			...
			tiPA += nStatesSquared;
			clL += numAVXChars*nStates - charNum[id]*nStates;
			clA += numAVXChars*nStates - charNum[id]*nStates;
			clR += numAVXChars*nStates - charNum[id]*nStates;
			clP += numAVXChars*nStates - charNum[id]*nStates;
	...
	case 0:
		...
		for (k=0; k<numGammaCats; k++)
			{
			for (c=clr_gen_AVX_prmt[id].c, t=0; c<(clr_gen_AVX_prmt[id].c+charNum[id]); c++)
				{
				for (c1=0; c1<FLOATS_PER_VEC; c1++,t++)
					{
					preLikeAV[c1] = &preLikeA[aState[t] + k*(preLikeJump+nStates)];
...
#   endif
    ...
    for(i=1; i<NUM_THREADS; i++)
    {
        ...
        clr_nuc4_prmt[i].k = 0;
		clr_nuc4_prmt[i].h = 0;
        clr_nuc4_prmt[i].shortCut = shortCut;
        clr_nuc4_prmt[i].pL = pL;
        clr_nuc4_prmt[i].pR = pR;
		clr_nuc4_prmt[i].pA = pA;
        clr_nuc4_prmt[i].clP = clP;
        clr_nuc4_prmt[i].clL = clL;
        clr_nuc4_prmt[i].clR = clR;
		clr_nuc4_prmt[i].clA = clA;
        clr_nuc4_prmt[i].lState = lState;
        clr_nuc4_prmt[i].rState = rState;
		clr_nuc4_prmt[i].aState = aState;
		clr_nuc4_prmt[i].numChars = m->numChars;
		clr_nuc4_prmt[i].numGammaCats = m->numGammaCats;
        for(j=0;j<i;j++)
        {
		clr_nuc4_prmt[i].c += charNum[j];
		clr_nuc4_prmt[i].h += (4*charNum[j]);
		clr_nuc4_prmt[i].clL += (4*charNum[j]);
		clr_nuc4_prmt[i].clR += (4*charNum[j]);
		clr_nuc4_prmt[i].clA += (4*charNum[j]);
		}
    }
    if(on_off == 0)
    {
...
void       ThreadCLRoot_NUC4 (int id)   
{
	int             a, c, h, i, j, k, shortCut, *lState=NULL, *rState=NULL, *aState=NULL;
    CLFlt           *clL, *clR, *clP, *clA, *pL, *pR, *pA, *tiPL, *tiPR, *tiPA;
	pL = clr_nuc4_prmt[id].pL;
    pR = clr_nuc4_prmt[id].pR;
	pA = clr_nuc4_prmt[id].pA;
    clL = clr_nuc4_prmt[id].clL;
    clR = clr_nuc4_prmt[id].clR;
    clP = clr_nuc4_prmt[id].clP;
	clA = clr_nuc4_prmt[id].clA;
    lState = clr_nuc4_prmt[id].lState;
    rState = clr_nuc4_prmt[id].rState;
	aState = clr_nuc4_prmt[id].aState;
	shortCut = clr_nuc4_prmt[id].shortCut;
	h = clr_nuc4_prmt[id].h;
    //k = cld_gen_prmt[id].k;
	//c = cld_gen_prmt[id].c;
	int numChars = clr_nuc4_prmt[id].numChars;
	int numGammaCats = clr_nuc4_prmt[id].numGammaCats;
	switch (shortCut)
        {
    case 4:
        tiPL = pL;
        ...
        for (k=0; k<numGammaCats; k++)
            {
				for (c=clr_nuc4_prmt[id].c; c<(clr_nuc4_prmt[id].c+charNum[id]); c++)
                {
                clP[h++] =   (tiPL[AA]*clL[A] + tiPL[AC]*clL[C] + tiPL[AG]*clL[G] + tiPL[AT]*clL[T])
                            *(tiPR[AA]*clR[A] + tiPR[AC]*clR[C] + tiPR[AG]*clR[G] + tiPR[AT]*clR[T])
                            *(tiPA[AA]*clA[A] + tiPA[AC]*clA[C] + tiPA[AG]*clA[G] + tiPA[AT]*clA[T]);
...
int CondLikeRoot_NUC4_AVX (TreeNode *p, int division, int chain)
{
	...
	for(int i=1;i<NUM_THREADS;++i)
	{
		clr_nuc4_AVX_prmt[i].clL = clL;
		clr_nuc4_AVX_prmt[i].clR = clR;
		clr_nuc4_AVX_prmt[i].clP = clP;
		clr_nuc4_AVX_prmt[i].clA = clA;
		clr_nuc4_AVX_prmt[i].pL = pL;
		clr_nuc4_AVX_prmt[i].pR = pR;
		clr_nuc4_AVX_prmt[i].pA = pA;
		clr_nuc4_AVX_prmt[i].c = 0;
		clr_nuc4_AVX_prmt[i].numAVXChars = m->numAVXChars;
		clr_nuc4_AVX_prmt[i].numGammaCats = m->numGammaCats;
		for(int j=0;j<i;++j)
		{
			clr_nuc4_AVX_prmt[i].c+=charNum[j];
			clr_nuc4_AVX_prmt[i].clL+=(4*charNum[j]);
			clr_nuc4_AVX_prmt[i].clR+=(4*charNum[j]);
			clr_nuc4_AVX_prmt[i].clP+=(4*charNum[j]);
			clr_nuc4_AVX_prmt[i].clA+=(4*charNum[j]);
		}
	}
	if(on_off==0)
	{	
    ...
    for (k=0; k<m->numGammaCats; k++)
		...
		clR += 4*m->numAVXChars - 4*charNum[0];
		clP += 4*m->numAVXChars - 4*charNum[0];
		clA += 4*m->numAVXChars - 4*charNum[0];
#endif 
        }
#ifdef FINE_GRAINED
	pthread_barrier_wait(&stop_barrier);
...
/*----------------------------------------------------------------
|
|   ThreadCLRoot_NUC4_AVX: 4by4 nucleotide model with or without rate
|       variation using AVX instructions (child thread view)
|
-----------------------------------------------------------------*/
int ThreadCLRoot_NUC4_AVX (int id)
{
	...
	clL = clr_nuc4_AVX_prmt[id].clL;
	clA = clr_nuc4_AVX_prmt[id].clA;
	clR = clr_nuc4_AVX_prmt[id].clR;
	clP = clr_nuc4_AVX_prmt[id].clP;
	int numAVXChars = clr_nuc4_AVX_prmt[id].numAVXChars;
	int numGammaCats = clr_nuc4_AVX_prmt[id].numGammaCats;
	pL = clr_nuc4_AVX_prmt[id].pL;
	pR = clr_nuc4_AVX_prmt[id].pR;
	pA = clr_nuc4_AVX_prmt[id].pA;
    ...
    for (k=0; k<numGammaCats; k++)
		...
		for (c=clr_nuc4_AVX_prmt[id].c; c<(clr_nuc4_AVX_prmt[id].c+charNum[id]); c++)
            {
            m1 = _mm256_set1_ps (tiPL[AA]);
            m2 = _mm256_set1_ps (tiPR[AA]);
            m3 = _mm256_set1_ps (tiPA[AA]);
		...
		clR += 4*numAVXChars - 4*charNum[id];
		clP += 4*numAVXChars - 4*charNum[id];
		clA += 4*numAVXChars - 4*charNum[id];
...
/*----------------------------------------------------------------
|
|   CondLikeRoot_NY98_AVX: codon model with omega variation
|
-----------------------------------------------------------------*/
        ...
        case 3:
            for (k=0; k<m->numOmegaCats; k++)
                ...
                for (c=t=0; c<m->numAVXChars; c++)
                    ...
                    for (i=0; i<nStates; i++)
                        {
                        assert (FLOATS_PER_VEC == 8); /* In the following 2 statements we assume that SSE register can hold exactly 4 ClFlts. */
                        mL = _mm256_set_ps (*(preLikeLV[7]++), *(preLikeLV[6]++), *(preLikeLV[5]++), *(preLikeLV[4]++),
                                            *(preLikeLV[3]++), *(preLikeLV[2]++), *(preLikeLV[1]++), *(preLikeLV[0]++));
                        mR = _mm256_set_ps (*(preLikeRV[7]++), *(preLikeRV[6]++), *(preLikeRV[5]++), *(preLikeRV[4]++),
                                            *(preLikeRV[3]++), *(preLikeRV[2]++), *(preLikeRV[1]++), *(preLikeRV[0]++));
...
int CondLikeScaler_Gen (TreeNode *p, int division, int chain)
{
    ...
    clP   = m->clP;
#ifdef FINE_GRAINED
	mclP = (CLFlt**)SafeMalloc(sizeof(CLFlt)*nStates*m->numChars*m->numGammaCats);
    memcpy(mclP, m->clP, sizeof(m->clP));
 #endif   
    ...
    for(int i = 1; i < NUM_THREADS; ++i)
    {
        cls_gen_prmt[i].scP = scP;
        cls_gen_prmt[i].lnScaler = lnScaler;
		cls_gen_prmt[i].clP = (CLFlt**)SafeMalloc(sizeof(CLFlt)*nStates*m->numChars*m->numGammaCats);
		memcpy(cls_gen_prmt[i].clP, mclP, sizeof(m->clP));
		cls_gen_prmt[i].clPtr = m->condLikes[m->condLikeIndex[chain][p->index]];
        cls_gen_prmt[i].c = 0;
        cls_gen_prmt[i].numChars = m->numChars;
        cls_gen_prmt[i].numGammaCats = m->numGammaCats;
        cls_gen_prmt[i].nStates = nStates;
        for(int j = 0; j < i; ++j)
        {
            cls_gen_prmt[i].c += charNum[j];
            cls_gen_prmt[i].clPtr += charNum[j]*nStates;
        }
        for (k=0; k<m->numGammaCats; k++)
        {
        cls_gen_prmt[i].clP[k] = cls_gen_prmt[i].clPtr;
        cls_gen_prmt[i].clPtr += m->numChars * nStates;
        }
    }
...
void ThreadCLScaler_Gen(int id)
{
    ...
    c = cls_gen_prmt[id].c;
    clP = cls_gen_prmt[id].clP;
    scP = cls_gen_prmt[id].scP;
    lnScaler = cls_gen_prmt[id].lnScaler;
    int numChars = cls_gen_prmt[id].numChars;
    int numGammaCats = cls_gen_prmt[id].numGammaCats;
    int nStates = cls_gen_prmt[id].nStates;
    for (c = cls_gen_prmt[id].c; c<(cls_gen_prmt[id].c + charNum[id]); ++c)
        {
        scaler = 0.0;
        for (k=0; k<numGammaCats; k++)
            {
...
/*----------------------------------------------------------------
|
|   CondLikeScaler_Gen_AVX: general n-state model with or without rate
|       variation
|
-----------------------------------------------------------------*/
...
#   endif
...
#ifdef FINE_GRAINED
	mclP = (__m256**)SafeMalloc(sizeof(__m256 *)*nStates*m->numAVXChars*m->numGammaCats);
    memcpy(mclP, m->clP_AVX, sizeof(m->clP_AVX));
 #endif
 
    for (k=0; k<m->numGammaCats; k++)
        {
        clP[k] = clPtr;
        clPtr += m->numAVXChars * m->numModelStates;
        }
    ...
    lnScaler = m->scalers[m->siteScalerIndex[chain]];
	scP_AVX = (__m256 *) scP;
	lnScaler_AVX = (__m256 *)lnScaler;
	
#ifdef FINE_GRAINED
    option = CLSCALER;
    for(int i = 1; i < NUM_THREADS; ++i)
    {
        cls_gen_AVX_prmt[i].scP_AVX = scP_AVX;
        cls_gen_AVX_prmt[i].lnScaler = lnScaler;
		cls_gen_AVX_prmt[i].clP = (__m256**)SafeMalloc(sizeof(__m256 *)*nStates*m->numAVXChars*m->numGammaCats);
		memcpy(cls_gen_AVX_prmt[i].clP, mclP, sizeof(m->clP_AVX));
		cls_gen_AVX_prmt[i].clPtr = (__m256 *)m->condLikes[m->condLikeIndex[chain][p->index]];
        cls_gen_AVX_prmt[i].c = 0;
        cls_gen_AVX_prmt[i].numAVXChars = m->numAVXChars;
        cls_gen_AVX_prmt[i].numGammaCats = m->numGammaCats;
		cls_gen_AVX_prmt[i].c1 = 0;
		cls_gen_AVX_prmt[i].scP = scP;
		cls_gen_AVX_prmt[i].lnScaler_AVX = lnScaler_AVX;
		cls_gen_AVX_prmt[i].nStates = nStates;
        for(int j = 0; j < i; ++j)
        {
            cls_gen_AVX_prmt[i].c += charNum[j];
			cls_gen_AVX_prmt[i].c1 += charNumScaler[j];
            cls_gen_AVX_prmt[i].clPtr += charNum[j]*nStates;
        }
        for (k=0; k<m->numGammaCats; k++)
        {
			cls_gen_AVX_prmt[i].clP[k] = cls_gen_AVX_prmt[i].clPtr;
			cls_gen_AVX_prmt[i].clPtr += m->numAVXChars * nStates;
        }
    }
   ...
   free(mclP);
    ...
    for (c=0; c<m->numAVXChars; c++)
        ...
        for (k=0; k<m->numGammaCats; k++)
            ...
            for (n=0; n<nStates; n++)
                {
                m1 = _mm256_max_ps (m1, clP[k][n]);
                }
            }
#if !defined (FINE_GRAINED)
        _mm256_store_ps (scP,  m1);
        scP += FLOATS_PER_VEC;
#endif	 
...
#ifdef FINE_GRAINED
		scP_AVX[c] = m1;
        scP_AVX[c] = log256_ps(scP_AVX[c]);
		lnScaler_AVX[c] = _mm256_add_ps(lnScaler_AVX[c],scP_AVX[c]);
...
int ThreadCLScaler_Gen_AVX(int id)
{
	int             c, k, n, nStates;
    CLFlt           *scP, *lnScaler;
    __m256          *clPtr, **clP, m1, *scP_AVX, *lnScaler_AVX;
#   if defined (FAST_LOG)
    int             index;
#   endif

    clP = cls_gen_AVX_prmt[id].clP;
	int numAVXChars = cls_gen_AVX_prmt[id].numAVXChars;
	int numGammaCats = cls_gen_AVX_prmt[id].numGammaCats;
	scP_AVX = cls_gen_AVX_prmt[id].scP_AVX;
	lnScaler = cls_gen_AVX_prmt[id].lnScaler;
	scP = cls_gen_AVX_prmt[id].scP;
	lnScaler_AVX = cls_gen_AVX_prmt[id].lnScaler_AVX;
	nStates = cls_gen_AVX_prmt[id].nStates;

    /* rescale */
    for (c=cls_gen_AVX_prmt[id].c; c<(cls_gen_AVX_prmt[id].c+charNum[id]); c++)
        {
        //scaler = 0.0;
        m1 = _mm256_setzero_ps ();
        for (k=0; k<numGammaCats; k++)
...
#   endif
...
#ifdef FINE_GRAINED
	mclP = (CLFlt**)malloc(sizeof(CLFlt)*4*m->numChars*m->numGammaCats);
    memcpy(mclP, m->clP, sizeof(m->clP));
 #endif
    ...
    for(int i = 1; i < NUM_THREADS; ++i)
    {
        cls_nuc4_prmt[i].scP = scP;
        cls_nuc4_prmt[i].lnScaler = lnScaler;
		cls_nuc4_prmt[i].clP = (CLFlt**)malloc(sizeof(CLFlt)*4*m->numChars*m->numGammaCats);
		memcpy(cls_nuc4_prmt[i].clP, mclP, sizeof(m->clP));
		//cls_gen_prmt[i].clP = mclP;
		cls_nuc4_prmt[i].clPtr = m->condLikes[m->condLikeIndex[chain][p->index]];
        cls_nuc4_prmt[i].c = 0;
        cls_nuc4_prmt[i].numChars = m->numChars;
        cls_nuc4_prmt[i].numGammaCats = m->numGammaCats;
        for(int j = 0; j < i; ++j)
        {
            cls_nuc4_prmt[i].c += charNum[j];
            cls_nuc4_prmt[i].clPtr += charNum[j]*4;
        }
        for (k=0; k<m->numGammaCats; k++)
        {
        cls_nuc4_prmt[i].clP[k] = cls_nuc4_prmt[i].clPtr;
        cls_nuc4_prmt[i].clPtr += m->numChars * 4;
        }
    }
...
void ThreadCLScaler_NUC4(int id)
{
    int c,k,n;
    CLFlt scaler,**clP,*scP,*lnScaler;
    c = cls_nuc4_prmt[id].c;
    clP = cls_nuc4_prmt[id].clP;
    scP = cls_nuc4_prmt[id].scP;
    lnScaler = cls_nuc4_prmt[id].lnScaler;
    int numChars = cls_nuc4_prmt[id].numChars;
    int numGammaCats = cls_nuc4_prmt[id].numGammaCats;
     /* rescale values */

    for (c=cls_nuc4_prmt[id].c; c<cls_nuc4_prmt[id].c+charNum[id]; c++)
        {
        scaler = 0.0;
        for (k=0; k<numGammaCats; k++)
            {
...
int CondLikeScaler_NUC4_AVX (TreeNode *p, int division, int chain)
{
    int             c, k;
    CLFlt           *scP, *lnScaler;
    __m256          *clPtr, **clP, *scP_AVX, m1, *lnScaler_AVX;
    ModelInfo       *m;
...
#ifdef FINE_GRAINED
	mclP = (__m256**)SafeMalloc(sizeof(__m256 *)*4*m->numAVXChars*m->numGammaCats);
    memcpy(mclP, m->clP_AVX, sizeof(m->clP_AVX));
 #endif
    ...
    for(int i = 1; i < NUM_THREADS; ++i)
    {
        cls_nuc4_AVX_prmt[i].scP_AVX = scP_AVX;
        cls_nuc4_AVX_prmt[i].lnScaler = lnScaler;
		cls_nuc4_AVX_prmt[i].clP = (__m256**)SafeMalloc(sizeof(__m256 *)*4*m->numAVXChars*m->numGammaCats);
		memcpy(cls_nuc4_AVX_prmt[i].clP, mclP, sizeof(m->clP_AVX));
		cls_nuc4_AVX_prmt[i].clPtr = (__m256 *)m->condLikes[m->condLikeIndex[chain][p->index]];
        cls_nuc4_AVX_prmt[i].c = 0;
        cls_nuc4_AVX_prmt[i].numAVXChars = m->numAVXChars;
        cls_nuc4_AVX_prmt[i].numGammaCats = m->numGammaCats;
		cls_nuc4_AVX_prmt[i].c1 = 0;
		cls_nuc4_AVX_prmt[i].scP = scP;
		cls_nuc4_AVX_prmt[i].lnScaler_AVX = lnScaler_AVX;
        for(int j = 0; j < i; ++j)
        {
            cls_nuc4_AVX_prmt[i].c += charNum[j];
			cls_nuc4_AVX_prmt[i].c1 += charNumScaler[j];
            cls_nuc4_AVX_prmt[i].clPtr += charNum[j]*4;
			//cls_nuc4_AVX_prmt[i].scP_AVX+= charNum[j];
        }
        for (k=0; k<m->numGammaCats; k++)
        {
        cls_nuc4_AVX_prmt[i].clP[k] = cls_nuc4_AVX_prmt[i].clPtr;
        cls_nuc4_AVX_prmt[i].clPtr += m->numAVXChars * 4;
        }
    }
   ...
   free(mclP);
    ...
    for (c=0; c<m->numChars; c++)
	{			
		lnScaler[c] += (scP[c] = (CLFlt)(log (scP[c]))); 
	}
#endif
    /* add log of new scaler into tree scaler  */
#ifdef FINE_GRAINED
	pthread_barrier_wait(&stop_barrier);
	
	for(int i = 1; i< NUM_THREADS; ++i)
	{
		free(cls_nuc4_AVX_prmt[i].clP);
	}
#endif
...
int ThreadCLScaler_NUC4_AVX (int id)
{
	...
	clP = cls_nuc4_AVX_prmt[id].clP;
	int numAVXChars = cls_nuc4_AVX_prmt[id].numAVXChars;
	int numGammaCats = cls_nuc4_AVX_prmt[id].numGammaCats;
	scP_AVX = cls_nuc4_AVX_prmt[id].scP_AVX;
	lnScaler = cls_nuc4_AVX_prmt[id].lnScaler;
	scP = cls_nuc4_AVX_prmt[id].scP;
	lnScaler_AVX = cls_nuc4_AVX_prmt[id].lnScaler_AVX;
    
    /* rescale */
	for(c=cls_nuc4_AVX_prmt[id].c;c<(cls_nuc4_AVX_prmt[id].c+charNum[id]);c++)
        {
        m1 = _mm256_setzero_ps ();
        for (k=0; k<numGammaCats; k++)
            {
...
/*----------------------------------------------------------------
|
|   CondLikeScaler_NY98_AVX: codon model with omega variation
|
-----------------------------------------------------------------*/
...
int Likelihood_Gen (TreeNode *p, int division, int chain, MrBFlt *lnL, int whichSitePats)
{
...
#ifdef FINE_GRAINED
	mclP = (CLFlt**)malloc(sizeof(CLFlt)*nStates*m->numChars*m->numGammaCats);
    memcpy(mclP, m->clP, sizeof(m->clP));
 #endif  
    ...
    *lnL = 0.0;
	...
	for(int i=1; i<NUM_THREADS; ++i)
	{
		likeh_gen_prmt[i].clP = (CLFlt**)malloc(sizeof(CLFlt)*nStates*m->numChars*m->numGammaCats);
		memcpy(likeh_gen_prmt[i].clP, mclP, sizeof(m->clP));
		likeh_gen_prmt[i].clPtr = m->condLikes[m->condLikeIndex[chain][p->index]];
		likeh_gen_prmt[i].lnScaler = lnScaler;
		likeh_gen_prmt[i].nSitesOfPat = nSitesOfPat;
		likeh_gen_prmt[i].clInvar = clInvar;
		likeh_gen_prmt[i].hasPInvar = hasPInvar;
		likeh_gen_prmt[i].numChars = m->numChars;
		likeh_gen_prmt[i].numGammaCats = m->numGammaCats;
		likeh_gen_prmt[i].nStates = nStates;
		//likeh_gen_prmt[i].lnL = lnL;
		likeh_gen_prmt[i].freq = freq;
		likeh_gen_prmt[i].bs = bs;
		likeh_gen_prmt[i].pInvar = pInvar;
	    likeh_gen_prmt[i].c = 0;
		for(int j = 0; j < i; ++j)
		{
			likeh_gen_prmt[i].c += charNum[j];
			likeh_gen_prmt[i].clPtr += charNum[j]*nStates;
			likeh_gen_prmt[i].clInvar += charNum[j]* nStates;
			
		}
		for (k=0; k<m->numGammaCats; k++)
		{
		likeh_gen_prmt[i].clP[k] = likeh_gen_prmt[i].clPtr;
		likeh_gen_prmt[i].clPtr += m->numChars * nStates;
		}
	}
	pthread_barrier_wait(&start_barrier);
    free(mclP);
	...
	for(int i=1; i< NUM_THREADS; ++i)
	{
		(*lnL) += likeh_gen_prmt[i].lnL;
		free(likeh_gen_prmt[i].clP);
	}
#endif
...
{
	int             c, j, k, nStates, hasPInvar;
    MrBFlt          freq, *bs, like, likeI, pInvar=0.0, lnLike;
    CLFlt           **clP, *lnScaler, *nSitesOfPat, *clInvar=NULL;
	MrBFlt          lnL;
	lnL = 0.0;
	nStates = likeh_gen_prmt[id].nStates;
	hasPInvar = likeh_gen_prmt[id].hasPInvar;
	freq = likeh_gen_prmt[id].freq;
	bs = likeh_gen_prmt[id].bs;
	pInvar = likeh_gen_prmt[id].pInvar;
	clP = likeh_gen_prmt[id].clP;
	lnScaler = likeh_gen_prmt[id].lnScaler;
	nSitesOfPat = likeh_gen_prmt[id].nSitesOfPat;
	clInvar = likeh_gen_prmt[id].clInvar;
	int numGammaCats = likeh_gen_prmt[id].numGammaCats;
	if (hasPInvar == NO)
        {
			for(c = likeh_gen_prmt[id].c; c< (likeh_gen_prmt[id].c + charNum[id]); ++c)
			{
				like = 0.0;
				for (k=0; k<numGammaCats; k++)
					for (j=0; j<nStates; j++)
...
/*------------------------------------------------------------------
|
|   Likelihood_Gen_AVX: general n-state model with or without rate
|       variation
|
-------------------------------------------------------------------*/
int Likelihood_Gen_AVX (TreeNode *p, int division, int chain, MrBFlt *lnL, int whichSitePats)
{
    ...
    MrBFlt          like, *bs;
    MrBFlt          s01, s10, probOn, probOff, *swr, covBF[40], freq, likeI, pInvar=0.0, lnLike;
    CLFlt           *lnScaler, *nSitesOfPat, *lnL_AVX, *lnLI_AVX;
    __m256          *clPtr, **clP, *clInvar=NULL;
    __m256          m1, mCatLike, mLike, mFreq;
    ModelInfo       *m;
    ...
    *lnL = 0.0;
    ...
    for (c=0; c<m->numAVXChars; c++)
        ...
        for (k=0; k<m->numGammaCats; k++)
            {
            mCatLike = _mm256_setzero_ps ();
            for (j=0; j<nStates; j++)
                {
                m1 = _mm256_mul_ps (clP[k][j], _mm256_set1_ps ((CLFlt)bs[j]));
                mCatLike = _mm256_add_ps (mCatLike, m1);
                }
            m1 = _mm256_mul_ps (mCatLike, mFreq);
            mLike = _mm256_add_ps (mLike, m1);
            clP[k] += nStates;
            }
        _mm256_store_ps (lnL_AVX, mLike);
        lnL_AVX += FLOATS_PER_VEC;
        }
...
#   endif
        ...
        for (c=0; c<m->numAVXChars; c++)
            ...
            for (j=0; j<nStates; j++)
                {
                m1 = _mm256_mul_ps (clInvar[j], _mm256_set1_ps ((CLFlt)bs[j]));
                mCatLike = _mm256_add_ps (mCatLike, m1);
                }
            clInvar += nStates;
            _mm256_store_ps (lnL_AVX, mCatLike);
            lnLI_AVX += FLOATS_PER_VEC;
            }
...
void       ThreadLikeH_NUC4 (int id)
{
	...
	lnL = 0.0;
	//nStates = likeh_gen_prmt[id].nStates;
	hasPInvar = likeh_nuc4_prmt[id].hasPInvar;
	freq = likeh_nuc4_prmt[id].freq;
	bs = likeh_nuc4_prmt[id].bs;
	pInvar = likeh_nuc4_prmt[id].pInvar;
	clP = likeh_nuc4_prmt[id].clP;
	lnScaler = likeh_nuc4_prmt[id].lnScaler;
	nSitesOfPat = likeh_nuc4_prmt[id].nSitesOfPat;
	clInvar = likeh_nuc4_prmt[id].clInvar;
	int numGammaCats = likeh_nuc4_prmt[id].numGammaCats;
	if (hasPInvar == NO)
        {
			for(c = likeh_nuc4_prmt[id].c; c< (likeh_nuc4_prmt[id].c + charNum[id]); ++c)
			{
				like = 0.0;
            for (k=0; k<numGammaCats; k++)
                {
...
#   endif
    ...
    *lnL = 0.0;
	...
	for(int i=1; i<NUM_THREADS; ++i)
	{
		likeh_nuc4_prmt[i].clP = (CLFlt**)malloc(sizeof(CLFlt)*4*m->numChars*m->numGammaCats);
		memcpy(likeh_nuc4_prmt[i].clP, mclP, sizeof(m->clP));
		likeh_nuc4_prmt[i].clPtr = m->condLikes[m->condLikeIndex[chain][p->index]];
		likeh_nuc4_prmt[i].lnScaler = lnScaler;
		likeh_nuc4_prmt[i].nSitesOfPat = nSitesOfPat;
		likeh_nuc4_prmt[i].clInvar = clInvar;
		likeh_nuc4_prmt[i].hasPInvar = hasPInvar;
		likeh_nuc4_prmt[i].numChars = m->numChars;
		likeh_nuc4_prmt[i].numGammaCats = m->numGammaCats;
		likeh_nuc4_prmt[i].freq = freq;
		likeh_nuc4_prmt[i].bs = bs;
		likeh_nuc4_prmt[i].pInvar = pInvar;
	    likeh_nuc4_prmt[i].c = 0;
		for(int j = 0; j < i; ++j)
		{
			likeh_nuc4_prmt[i].c += charNum[j];
			likeh_nuc4_prmt[i].clPtr += charNum[j]*4;
			likeh_nuc4_prmt[i].clInvar += charNum[j]* 4;
			
		}
		for (k=0; k<m->numGammaCats; k++)
		{
		likeh_nuc4_prmt[i].clP[k] = likeh_nuc4_prmt[i].clPtr;
		likeh_nuc4_prmt[i].clPtr += m->numChars * 4;
		}
	}
	pthread_barrier_wait(&start_barrier);
    free(mclP);
...
#   endif
	...
	for(int i=1; i< NUM_THREADS; ++i)
	{
		(*lnL) += likeh_nuc4_prmt[i].lnL;
		free(likeh_nuc4_prmt[i].clP);
	}
#endif
    ...
    *lnL = 0.0;
    ...
    if (hasPInvar == YES)
        ...
        for (c=0; c<m->numAVXChars; c++)
            ...
            _mm256_store_ps (lnLI_AVX, mLike);
            clInvar += 4;
            lnLI_AVX += FLOATS_PER_VEC;
            }
        }
...
/*------------------------------------------------------------------
|
|   Likelihood_NY98_AVX: Codon model with three selection categories,
|       after Nielsen and Yang (1998).
|
-------------------------------------------------------------------*/
int Likelihood_NY98_AVX (TreeNode *p, int division, int chain, MrBFlt *lnL, int whichSitePats)
{
    int             c, j, k, nStates;
    MrBFlt          like, *bs, *omegaCatFreq;
    CLFlt           *lnScaler, *nSitesOfPat, *lnL_AVX;
    __m256          *clPtr, **clP;
    __m256          m1, mCatLike, mLike;
    ModelInfo       *m;
    ...
    *lnL = 0.0; /* reset lnL */
    ...
    for (c=0; c<m->numAVXChars; c++)
        ...
        for (k=0; k<m->numOmegaCats; k++)
            ...
            m1 = _mm256_mul_ps (mCatLike, _mm256_set1_ps ((CLFlt)omegaCatFreq[k]));
            mLike = _mm256_add_ps (mLike, m1);
            clP[k] += nStates;
            }
        _mm256_store_ps (lnL_AVX, mLike);
...
/*----------------------------------------------------------------
|
|   RemoveNodeScalers_SSE: Remove node scalers, AVX code
|
-----------------------------------------------------------------*/
int RemoveNodeScalers_AVX (TreeNode *p, int division, int chain)
{
...
{
    register int    i, j, k, n, s, index;
    MrBFlt          t, *catRate, baseRate, *eigenValues __attribute__((aligned(32))), *cijk __attribute__((aligned(32))), 
                    *bs __attribute__((aligned(32))),EigValexp[64] __attribute__((aligned(32))), sum, 
                    *ptr __attribute__((aligned(32))), theRate, correctionFactor,
                    length;
    CLFlt           *tiP __attribute__((aligned(32)));
    ModelInfo       *m __attribute__((aligned(32)));
    ...
    for (k = index = 0; k < m->numGammaCats; k++)
    {
        ...
        {
            /* Fill in identity matrix */
            int sz = sizeof(CLFlt) * n;
            for (i = 0; i < end; i += 8)
            {
                memset(&tiP[i], 0, sz);//i-th Row
                tiP[i * n + i] = 1.0f;
                memset(&tiP[i + 1], 0, sz);
                tiP[(i + 1) * n + i + 1] = 1.0f;
                memset(&tiP[i + 2], 0, sz);
        ...
        else if (t > TIME_MAX)
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                {
                    tiP[index++] = (CLFlt) bs[j];
                    tiP[index++] = (CLFlt) bs[j + 1];
                    tiP[index++] = (CLFlt) bs[j + 2];
                    tiP[index++] = (CLFlt) bs[j + 3];

                    tiP[index++] = (CLFlt) bs[j + 4];
                    tiP[index++] = (CLFlt) bs[j + 5];
                    tiP[index++] = (CLFlt) bs[j + 6];
                    tiP[index++] = (CLFlt) bs[j + 7];
                }
                for(j = end; j < n; ++j) tiP[index++] = (CLFlt) bs[j];
            }
        }
        else
        {
            ...
            {
                EigValexp[s] =  exp(eigenValues[s] * t);
                EigValexp[s + 1] =  exp(eigenValues[s + 1] * t);
                EigValexp[s + 2] =  exp(eigenValues[s + 2] * t);
                EigValexp[s + 3] =  exp(eigenValues[s + 3] * t);
                
                EigValexp[s + 4] =  exp(eigenValues[s + 4] * t);
                EigValexp[s + 5] =  exp(eigenValues[s + 5] * t);
                EigValexp[s + 6] =  exp(eigenValues[s + 6] * t);
                EigValexp[s + 7] =  exp(eigenValues[s + 7] * t);
            }
            for(s = end; s < n; ++s)  EigValexp[s] =  exp(eigenValues[s] * t);
            ...
            for (i = 0; i < nSquare; ++i)
            {
                ...
                {
                    sum += (*ptr++) * EigValexp[s];
                    sum += (*ptr++) * EigValexp[s + 1];
                    sum += (*ptr++) * EigValexp[s + 2];
                    sum += (*ptr++) * EigValexp[s + 3];

                    sum += (*ptr++) * EigValexp[s + 4];
                    sum += (*ptr++) * EigValexp[s + 5];
                    sum += (*ptr++) * EigValexp[s + 6];
                    sum += (*ptr++) * EigValexp[s + 7];
                }
                for(s = end; s < n; ++s) sum += (*ptr++) * EigValexp[s];
                tiP[index++] = (CLFlt) ((sum < 0.0) ? 0.0 : sum);
            }
        }
    }
...
/*------------------------------------------------------------------
|
|   Likelihood_Res_AVX: 4by4 nucleotide models with or without rate
|       variation
|
-------------------------------------------------------------------*/
int Likelihood_Res_AVX (TreeNode *p, int division, int chain, MrBFlt *lnL, int whichSitePats)
{
    ...
    /* find base frequencies */
    bs = GetParamSubVals (m->stateFreq, chain, state[chain]);
    mA = _mm256_set1_ps ((CLFlt)(bs[0]));
    mB = _mm256_set1_ps ((CLFlt)(bs[1]));
    ...
    *lnL = 0.0;
    ...
    for (c=0; c<m->numAVXChars; c++)
        ...
        for (k=0; k<m->numGammaCats; k++)
            {
            m1    = _mm256_mul_ps (clP[k][0], mA);
            mLike = _mm256_add_ps (mLike, m1);
            m1    = _mm256_mul_ps (clP[k][1], mB);
            mLike = _mm256_add_ps (mLike, m1);
            clP[k] += 2;
            }
        mLike = _mm256_mul_ps (mLike, mFreq);
