...
MrBFlt    GibbsSampleGammaThread (int chain, int division, RandLong *seed);
...
int       PosSelProbs_AVX (TreeNode *p, int division, int chain);
#endif
...
#endif
#if defined (AVX_ENABLED)
int       SiteOmegas_AVX (TreeNode *p, int division, int chain);
#endif
...
#	elif defined (AVX_ENABLED)
		if (m->useAVX == YES)
		{
			...
			if(m->lnL_AVX)
				AlignedSafeFree ((void **)(&m->lnL_AVX));
			if(m->lnLI_AVX)
				AlignedSafeFree((void **)(&m->lnLI_AVX));
		}
#   endif
...
MrBFlt GibbsSampleGamma (int chain, int division, RandLong *seed)
{
    ...
    m = &modelSettings[division];
//	MrBayesPrint ("start gibbssample", m->numChars);
...
/*
		...
		if(useBeagle == NO)
		{
			...
			else if (m->useAVX == YES)
			{
				MrBayesPrint ("%s   Using standard AVX likelihood calculator for division %d (single-precision)\n", spacer, d+1);
			}
			else
				MrBayesPrint ("%s   Using standard non-SSE and non-AVX likelihood calculator for division %d (%s-precision)\n", spacer, d+1, (sizeof(CLFlt) == 4 ? "single" : "double"));
		}
...
#	elif defined (AVX_ENABLED)
                if (m->useAVX == YES)
                    {
                    /* calculate number AVX chars */
                    m->numAVXChars = ((m->numChars - 1) / FLOATS_PER_VEC) + 1;
                    ...
                    if (m->gibbsGamma == YES)
                        ...
                        numReps = m->numGammaCats * m->numOmegaCats;
                    k = m->numAVXChars * FLOATS_PER_VEC * m->numModelStates * numReps;
...
#	elif defined (AVX_ENABLED)
				if (m->useAVX == YES)
                    {
                    /* allocate space with padding */
                    m->scalers[i] = (CLFlt*) AlignedMalloc (m->numAVXChars * FLOATS_PER_VEC * sizeof(CLFlt), 32);
                    if (!m->scalers[i])
                        return (ERROR);
                    for (j=0; j<m->numAVXChars*FLOATS_PER_VEC; j++)
                        m->scalers[i][j] = 0.0f;
                    }
                else
                    {
...
#	elif defined (AVX_ENABLED)
				if (m->useAVX == YES)
                    ...
                    if (!m->clP_AVX)
                        return (ERROR);
                    m->lnL_AVX  = AlignedMalloc (m->numAVXChars * FLOATS_PER_VEC * sizeof(CLFlt*), 32);
                    m->lnLI_AVX = AlignedMalloc (m->numAVXChars * FLOATS_PER_VEC * sizeof(CLFlt*), 32);
                    if (!m->lnL_AVX || !m->lnLI_AVX)
                        return (ERROR);
                    }
#   endif
                }
...
#	elif defined (AVX_ENABLED)
		c1 = m->numAVXChars * FLOATS_PER_VEC * m->numModelStates;
...
/*calculate index for each thread*/
	...
	/* if(on_off == 0)
	{ */
		//gammacatNum = m->numGammaCats;
		quotient=0,remain=0;
#ifdef AVX_ENABLED
		quotient = ((m->numAVXChars)/NUM_THREADS);
		remain = ((m->numAVXChars)%NUM_THREADS);
#elif defined SSE_ENABLED
		quotient = ((m->numSSEChars)/NUM_THREADS);
		remain = ((m->numSSEChars)%NUM_THREADS);
#else
		quotient = ((m->numChars)/NUM_THREADS);
		remain = ((m->numChars)%NUM_THREADS);
#endif
...
#if defined (AVX_ENABLED)
/* Calculate positive selection probabilities (AVX version) */
int PosSelProbs_AVX (TreeNode *p, int division, int chain)
{
    int             i, c1, c2, j, k, nStates;
    CLFlt           **catLike, *siteLike;
    MrBFlt          *bs, *omegaCatFreq, *omega,
                    posProb, *ps;
    __m256          m1, m2, *clPtr, **clP, mSiteLike, *mCatLike;
    __m256          m3;
    ModelInfo       *m;
    ...
    /* allocate space for category likelihood arrays */
    catLike = (CLFlt **) calloc (m->numOmegaCats, sizeof(CLFlt *));
    mCatLike = (__m256 *) calloc (m->numOmegaCats, sizeof(__m256));
    if (!catLike || !mCatLike)
        {
        MrBayesPrint ("%s   ERROR: Out of memory in PosSelProbs_AVX\n", spacer);
        return (ERROR);
        }
    ...
    /* find posSelProbs */
    ps = posSelProbs + m->compCharStart;
    for (c1=c2=0; c1<m->numAVXChars; c1++)
        {
        mSiteLike = _mm256_setzero_ps ();
        for (k=0; k<m->numOmegaCats; k++)
            {
            mCatLike[k] = _mm256_setzero_ps();
            m1 = _mm256_setzero_ps ();
            for (j=0; j<nStates; j++)
                {
                m2 = _mm256_mul_ps (clP[k][j], _mm256_set1_ps ((CLFlt)bs[j]));
                m1 = _mm256_add_ps (m1, m2);
                }

            m3=_mm256_set1_ps((CLFlt)omegaCatFreq[k]);
            MrBayesPrint ("%s   ERROR: Out of memory in SiteOmegas_AVX\n", spacer);
            mCatLike[k] = _mm256_mul_ps (m1, m3);
            mSiteLike = _mm256_add_ps (mSiteLike, mCatLike[k]);
            clP[k] += nStates;
            }
        
        //assert(FLOATS_PER_VEC==8);
        for (i=0; i<FLOATS_PER_VEC && c2 < m->numChars; ++i, ++c2)
            {
            assert(FLOATS_PER_VEC==8);
            posProb = 0.0;
...
int SiteOmegas_AVX(TreeNode *p, int division, int chain)
{
    ...
    for (c1=c2=0; c1<m->numAVXChars; c1++)
        ...
        for (k=0; k<m->numOmegaCats; k++)
            ...
            mCatLike[k] = _mm256_mul_ps (m1, _mm256_set1_ps ((CLFlt)omegaCatFreq[k]));
            mSiteLike = _mm256_add_ps (mSiteLike, mCatLike[k]);
            clP[k] += nStates;
            }
...
#   endif
    ...
    for (n=numPreviousGen+1; n<=chainParams.numGen; n++) /* begin run chain *////finish numGen times MCMC cycle
        {
...
#   endif
	...
	switch (datatype)
	{
		case GEN:
			...
			{
#if defined AVX_ENABLED
				pthread_join(clmix_gen_AVX_thd[i],NULL);
#else
				pthread_join(clmix_gen_thd[i],NULL);
#endif
			}
			break;
		case NUC4:
			...
			{
#if defined AVX_ENABLED
				pthread_join(clmix_nuc4_AVX_thd[i],NULL);
#else
				pthread_join(clmix_nuc4_thd[i],NULL);
#endif
			}
			break;
...
#   elif defined (AVX_ENABLED)
                        if (m->printAncStates == YES || m->printSiteRates == YES)
                            {
                            MrBayesPrint ("%s   Non-AVX version of conditional likelihood calculator will be used for division %d\n", spacer, i+1);
                            MrBayesPrint ("%s   due to request of reporting 'ancestral states' or 'site rates'.\n", spacer);
                            }
...
#	elif defined (AVX_ENABLED)
                            if (m->printAncStates == YES || m->printSiteRates == YES)
                                ...
                                {
                                m->useAVX= YES;
                                m->CondLikeDown = &CondLikeDown_Gen_AVX;
                                m->CondLikeRoot = &CondLikeRoot_Gen_AVX;
                                m->CondLikeScaler = &CondLikeScaler_Gen_AVX;
                                m->Likelihood = &Likelihood_Gen_AVX;
                                }								
#   endif
                            }
                        }
...
#   elif defined (AVX_ENABLED)
                        if (m->printAncStates == YES || m->printSiteRates == YES)
                            ...
                            {
                            m->useAVX= YES;
                            m->CondLikeDown   = &CondLikeDown_NY98_AVX;
                            m->CondLikeRoot   = &CondLikeRoot_NY98_AVX;
                            m->CondLikeScaler = &CondLikeScaler_NY98_AVX;
                            m->Likelihood     = &Likelihood_NY98_AVX;
                            m->PosSelProbs    = &PosSelProbs;
                            m->SiteOmegas     = &SiteOmegas;
                            }
#   endif
...
#   elif defined (AVX_ENABLED)
                if (m->printAncStates == YES || m->printSiteRates == YES)
                    ...
                    {
                    m->useAVX= YES;
                    m->CondLikeDown   = &CondLikeDown_Bin_AVX;
                    m->CondLikeRoot   = &CondLikeRoot_Bin_AVX;
                    m->CondLikeScaler = &CondLikeScaler_Gen_AVX;
                    m->Likelihood     = &Likelihood_Res_AVX;
                    }
...
# elif defined AVX_ENABLED
	for (d=0; d<numCurrentDivisions; d++)
        ...
        m->numAVXChars = ((m->numChars - 1) / FLOATS_PER_VEC) + 1;
        if (m->dataType != STANDARD && m->gibbsGamma == NO)
            numComprChars += m->numAVXChars * FLOATS_PER_VEC;
        else
            numComprChars += m->numChars;
        }
#   else
